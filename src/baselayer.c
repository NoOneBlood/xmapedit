...
static int osdcmd_vars(const osdfuncparm_t *parm)
{
	...
	if (!Bstrcasecmp(parm->name, "novoxmips"))
	{
	...
	else if (!Bstrcasecmp(parm->name, "fpgrouscan")) {
		if (showval) { buildprintf("fpgrouscan is %d\n", fpgrouscan); }
		else { fpgrouscan = (atoi(parm->parms[0]) != 0); }
		return OSDCMD_OK;
	}
#if defined(DEBUGGINGAIDS) && USE_OPENGL
...
int baselayer_init(void)
{
	...
	OSD_RegisterFunction("fpgrouscan", "fpgrouscan: enable/disable floating-point grouscan", osdcmd_vars);
