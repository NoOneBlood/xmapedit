...
void SCREEN2D::DrawWallMidPoint(int nWall, char c, char s, char thick, char which)
{
    ...
    if (OnScreen(cx, cy, s))
    {
        nSize = mulscale16(nLen, data.zoom);
        nSize = ClipRange(nSize, 2, 8);
...
void SCREEN2D::ScreenDraw(void)
{
    ...
    if (linehighlight >= 0)
    {
        ...
        {
            ScalePoints(&x1, &y1);
            gfxSetColor(GetWallColor(linehighlight, false));
            gfxPixel(x1-1, y1);
            gfxPixel(x1+1, y1);
            gfxPixel(x1, y1-1);
            gfxPixel(x1, y1+1);
            //DrawIconCross(x1, y1, GetWallColor(linehighlight, false), 1);
        }
    }
